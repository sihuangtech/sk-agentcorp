...
针对现有平台（如 Paperclip, Auto-Company）的卡死、上下文丢失、协调失败等痛点，SK AgentCorp 引入了**强状态持久化**、**全解耦配置生态**与强大的**防卡死引擎 (Anti-Stuck Engine)**。
...
- **多模型无缝切换 (Zero-Env Strategy)**：基于纯 JSON 动态文件路由，真正做到一行代码不改，开箱原生支持全球及中国 22 家大模型厂商网关！
...
- `backend/` - FastAPI 服务, LangGraph 工作流引擎, 防卡死模块, JSON 外部路由配置 (`llm_configs/`)
